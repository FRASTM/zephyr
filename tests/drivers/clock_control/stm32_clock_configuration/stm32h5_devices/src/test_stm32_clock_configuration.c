...
ZTEST(stm32h5_devices_clocks, test_sysclk_freq)
{
...
ZTEST(stm32h5_devices_clocks, test_spi_clk_config)
{
	static const struct stm32_pclken pclken[] = STM32_DT_CLOCKS(DT_NODELABEL(spi2));
	struct stm32_pclken spi2_reg_clk_cfg = pclken[0];

	uint32_t spi2_actual_domain_clk;
	uint32_t spi2_dt_clk_freq, spi2_actual_clk_freq;
	int r;

	/* Test clock_on(reg_clk) */
	r = clock_control_on(DEVICE_DT_GET(STM32_CLOCK_CONTROL_NODE),
				(clock_control_subsys_t) &spi2_reg_clk_cfg);
	zassert_true((r == 0), "Could not enable SPI reg_clk");

	zassert_true(__HAL_RCC_SPI2_IS_CLK_ENABLED(), "SPI2 reg_clk should be on");
	TC_PRINT("SPI2 reg_clk on\n");

	if (IS_ENABLED(STM32_SPI_DOMAIN_CLOCK_SUPPORT) && DT_NUM_CLOCKS(DT_NODELABEL(spi2)) > 1) {
		struct stm32_pclken spi2_domain_clk_cfg = pclken[1];

		/* Select domain_clk as device source clock */
		r = clock_control_configure(DEVICE_DT_GET(STM32_CLOCK_CONTROL_NODE),
					    (clock_control_subsys_t) &spi2_domain_clk_cfg,
					    NULL);
		zassert_true((r == 0), "Could not enable SPI domain_clk");
		TC_PRINT("SPI2 domain_clk on\n");

		spi2_actual_domain_clk = __HAL_RCC_GET_SPI2_SOURCE();

		if (pclken[1].bus == STM32_SRC_PLL1_Q) {
			zassert_equal(spi2_actual_domain_clk, RCC_SPI2CLKSOURCE_PLL1Q,
					"Expected SPI src: PLL1 Q (0x%x). Actual: 0x%x",
					RCC_SPI2CLKSOURCE_PLL1Q, spi2_actual_domain_clk);
		} else if (pclken[1].bus == STM32_SRC_PLL2_P) {
			zassert_equal(spi2_actual_domain_clk, RCC_SPI2CLKSOURCE_PLL2P,
					"Expected SPI src: PLL2 P (0x%x). Actual: 0x%x",
					RCC_SPI2CLKSOURCE_PLL2P, spi2_actual_domain_clk);
		} else if (pclken[1].bus == STM32_SRC_PLL3_P) {
			zassert_equal(spi2_actual_domain_clk, RCC_SPI2CLKSOURCE_PLL3P,
					"Expected SPI src: PLL3 P (0x%x). Actual: 0x%x",
					RCC_SPI2CLKSOURCE_PLL3P, spi2_actual_domain_clk);
		} else if (pclken[1].bus == STM32_SRC_CKPER) {
			zassert_equal(spi2_actual_domain_clk, RCC_SPI2CLKSOURCE_CLKP,
					"Expected SPI src: PERCLK (0x%x). Actual: 0x%x",
					RCC_SPI2CLKSOURCE_CLKP, spi2_actual_domain_clk);
			...
			if (perclk_dt_domain_clk == STM32_SRC_HSI) {
			...
			} else if (perclk_dt_domain_clk == STM32_SRC_CSI) {
		...
		spi2_actual_clk_freq = HAL_RCCEx_GetPeriphCLKFreq(RCC_PERIPHCLK_SPI2);
		zassert_equal(spi2_dt_clk_freq, spi2_actual_clk_freq,
				"Expected SPI clk: 0x%x. Actual SPI clk: 0x%x",
				spi2_dt_clk_freq, spi2_actual_clk_freq);
	} else {
	...
	TC_PRINT("SPI2 clock freq: %d MHz\n", spi2_actual_clk_freq / (1000*1000));
	...
	zassert_true(!__HAL_RCC_SPI2_IS_CLK_ENABLED(), "SPI2 reg_clk should be off");
	TC_PRINT("SPI2 reg_clk off\n");

	/* Test clock_off(domain_clk) */
	/* Not supported today */
}
ZTEST_SUITE(stm3257_devices_clocks, NULL, NULL, NULL, NULL, NULL);
