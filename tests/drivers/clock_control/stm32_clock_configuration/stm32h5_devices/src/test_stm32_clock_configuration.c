...
ZTEST(stm32h5_devices_clocks, test_spi_clk_config)
{
	...
	if (IS_ENABLED(STM32_SPI_DOMAIN_CLOCK_SUPPORT) && DT_NUM_CLOCKS(DT_NODELABEL(spi2)) > 1) {
		...
		if (pclken[1].bus == STM32_SRC_PLL1_Q) {
			zassert_equal(spi2_actual_domain_clk, RCC_SPI223CLKSOURCE_PLL,
					"Expected SPI src: PLL1 Q (0x%x). Actual: 0x%x",
					RCC_SPI223CLKSOURCE_PLL, spi2_actual_domain_clk);
		} else if (pclken[1].bus == STM32_SRC_PLL2_P) {
			zassert_equal(spi2_actual_domain_clk, RCC_SPI223CLKSOURCE_PLL2,
					"Expected SPI src: PLL2 P (0x%x). Actual: 0x%x",
					RCC_SPI223CLKSOURCE_PLL2, spi2_actual_domain_clk);
		} else if (pclken[1].bus == STM32_SRC_PLL3_P) {
			zassert_equal(spi2_actual_domain_clk, RCC_SPI223CLKSOURCE_PLL3,
					"Expected SPI src: PLL3 P (0x%x). Actual: 0x%x",
					RCC_SPI223CLKSOURCE_PLL3, spi2_actual_domain_clk);
		} else if (pclken[1].bus == STM32_SRC_CKPER) {
			zassert_equal(spi2_actual_domain_clk, RCC_SPI223CLKSOURCE_CLKP,
					"Expected SPI src: PERCLK (0x%x). Actual: 0x%x",
					RCC_SPI223CLKSOURCE_CLKP, spi2_actual_domain_clk);
