...
int main(void)
{
	...
	printf("\nTest 1: Flash erase page at 0x%x, size %d\n",
                TEST_PARTITION_OFFSET,
                TEST_PARTITION_SIZE);
	if (flash_erase(flash_dev, TEST_PARTITION_OFFSET,
                        TEST_PARTITION_SIZE) != 0) {
	...
	offset = TEST_PARTITION_OFFSET;
        ...
        for (i = 0; i < blocks; i++) {
                if (flash_write(flash_dev, offset + (i * FLASH_WRITE_SIZE),
        ...
        for (j = 0; j < blocks; j++) {
	        if (flash_read(flash_dev, offset + (j * FLASH_WRITE_SIZE),
                               &buffer_1[0], sizeof(buffer_1)) != 0) {
                ...
                if (j && ((j % 1024) == 0)) {
	                printf("   read and verified upto 0x%x\n",
