...
#define APB1_FREQ	((AHB_FREQ)/(STM32_D1PPRE))
#define APB2_FREQ	((AHB_FREQ)/(STM32_D2PPRE))
#define APB4_FREQ	((AHB_FREQ)/(STM32_D4PPRE))
#define APB5_FREQ	((AHB_FREQ)/(STM32_D5PPRE))
#else
#define SYSCLK_FREQ	((SYSCLKSRC_FREQ)/(STM32_D1CPRE))
#define AHB_FREQ	((SYSCLK_FREQ)/(STM32_HPRE))
...
#endif
#if defined(CONFIG_SOC_SERIES_STM32H7RSX)
	uint32_t apb1_clock = get_bus_clock(ahb_clock, STM32_D1PPRE);
	uint32_t apb2_clock = get_bus_clock(ahb_clock, STM32_D2PPRE);
	uint32_t apb4_clock = get_bus_clock(ahb_clock, STM32_D4PPRE);
	uint32_t apb5_clock = get_bus_clock(ahb_clock, STM32_D5PPRE);
#else
...
#if defined(CONFIG_SOC_SERIES_STM32H7RSX)
	LL_RCC_SetAPB1Prescaler(apb1_prescaler(STM32_D1PPRE));
	LL_RCC_SetAPB2Prescaler(apb2_prescaler(STM32_D2PPRE));
	LL_RCC_SetAPB4Prescaler(apb4_prescaler(STM32_D4PPRE));
	LL_RCC_SetAPB5Prescaler(apb5_prescaler(STM32_D5PPRE));
#else
