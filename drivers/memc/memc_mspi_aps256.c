...
#define APS256_WRITE                 0x02
#define APS256_READ                  0x03
#define APS256_FAST_READ             0x0B
#define APS256_QUAD_MODE_ENTER       0x35
#define APS256_QUAD_WRITE            0x38
#define APS256_RESET_ENABLE          0x66
#define APS256_RESET_MEMORY          0x99
#define APS256_READ_ID               0x9F
#define APS256_HALF_SLEEP_ENTER      0xC0
#define APS256_QUAD_READ             0xEB
#define APS256_QUAD_MODE_EXIT        0xF5

struct memc_mspi_aps256_config {
	uint32_t                       port;
	uint32_t                       mem_size;
...
static int memc_mspi_aps256_command_write(const struct device *psram, uint8_t cmd, uint32_t addr,
					    uint8_t *wdata, uint32_t length)
{
	const struct memc_mspi_aps256_config *cfg = psram->config;
	struct memc_mspi_aps256_data *data = psram->data;
	int ret;
	uint8_t buffer[16];
...
static int memc_mspi_aps256_command_read(const struct device *psram, uint8_t cmd, uint32_t addr,
...
static int memc_mspi_aps256_reset(const struct device *psram)
{
	int ret;

	LOG_DBG("Resetting aps256/%u", __LINE__);

	ret = memc_mspi_aps256_command_write(psram, APS256_RESET_ENABLE, 0, NULL, 0);
	if (ret) {
		return ret;
	}
	ret = memc_mspi_aps256_command_write(psram, APS256_RESET_MEMORY, 0, NULL, 0);
	if (ret) {
		return ret;
	}
	/** We need to delay 5 ms to allow aps256 pSRAM to reinitialize */
	k_busy_wait(5000);
	return ret;
}

static int memc_mspi_aps256_get_vendor_id(const struct device *psram, uint8_t *vendor_id)
{
	uint16_t buffer = 0;
	int ret;

	ret = memc_mspi_aps256_command_read(psram, APS256_READ_ID, 0, (uint8_t *)&buffer, 2);
...
#if CONFIG_PM_DEVICE
static int memc_mspi_aps256_half_sleep_enter(const struct device *psram)
{
	int ret;

	LOG_DBG("Putting aps256 to half sleep/%u", __LINE__);
	ret = memc_mspi_aps256_command_write(psram, APS256_HALF_SLEEP_ENTER, 0, NULL, 0);
	if (ret) {
...
static int memc_mspi_aps256_half_sleep_exit(const struct device *psram)
{
	const struct memc_mspi_aps256_config *cfg = psram->config;
	struct memc_mspi_aps256_data *data = psram->data;
	...
	LOG_DBG("Waking up aps256 from half sleep/%u", __LINE__);
	ret = memc_mspi_aps256_command_write(psram, 0, 0, NULL, 0);
	if (ret) {
...
static int memc_mspi_aps256_pm_action(const struct device *psram, enum pm_device_action action)
{
	switch (action) {
	case PM_DEVICE_ACTION_RESUME:
		acquire(psram);
		memc_mspi_aps256_half_sleep_exit(psram);
		release(psram);
		break;

	case PM_DEVICE_ACTION_SUSPEND:
		acquire(psram);
		memc_mspi_aps256_half_sleep_enter(psram);
		release(psram);
		break;
...
static int memc_mspi_aps256_init(const struct device *psram)
{
	const struct memc_mspi_aps256_config *cfg = psram->config;
	struct memc_mspi_aps256_data *data = psram->data;
	uint8_t vendor_id;
	...
	if (data->dev_cfg.io_mode == MSPI_IO_MODE_QUAD) {
		...
		if (memc_mspi_aps256_reset(psram)) {
			LOG_ERR("Could not reset pSRAM/%u", __LINE__);
			return -EIO;
		}
		if (memc_mspi_aps256_command_write(psram, APS256_QUAD_MODE_EXIT, 0, NULL, 0)) {
	...
	if (memc_mspi_aps256_get_vendor_id(psram, &vendor_id)) {
	...
	if (cfg->tar_dev_cfg.io_mode == MSPI_IO_MODE_QUAD) {
		if (memc_mspi_aps256_command_write(psram, APS256_QUAD_MODE_ENTER, 0, NULL, 0)) {
			return -EIO;
		}
	}
...
#define MSPI_DEVICE_CONFIG_SERIAL(n)                                                              \
	{                                                                                         \
		...
		.rx_dummy           = 8,                                                          \
		.tx_dummy           = 0,                                                          \
		.read_cmd           = APS256_FAST_READ,                                         \
		.write_cmd          = APS256_WRITE,                                             \
		.cmd_length         = 1,                                                          \
		.addr_length        = 3,                                                          \
		.mem_boundary       = 1024,                                                       \
		.time_to_break      = 8,                                                          \
...
#define MSPI_DEVICE_CONFIG_QUAD(n)                                                                \
	{                                                                                         \
		...
		.rx_dummy           = 6,                                                          \
		.tx_dummy           = 0,                                                          \
		.read_cmd           = APS256_QUAD_READ,                                         \
		.write_cmd          = APS256_QUAD_WRITE,                                        \
		.cmd_length         = 1,                                                          \
		.addr_length        = 3,                                                          \
		.mem_boundary       = 1024,                                                       \
		.time_to_break      = 4,                                                          \
...
#define MEMC_MSPI_APS256(n)                                                                     \
	static const struct memc_mspi_aps256_config                                             \
	memc_mspi_aps256_config_##n = {                                                         \
		.port               = MSPI_PORT(n),                                               \
	...
	static struct memc_mspi_aps256_data                                                     \
		memc_mspi_aps256_data_##n = {                                                   \
		.lock = Z_SEM_INITIALIZER(memc_mspi_aps256_data_##n.lock, 0, 1),                \
	};                                                                                        \
	PM_DEVICE_DT_INST_DEFINE(n, memc_mspi_aps256_pm_action);                                \
	DEVICE_DT_INST_DEFINE(n,                                                                  \
			      memc_mspi_aps256_init,                                            \
...
DT_INST_FOREACH_STATUS_OKAY(MEMC_MSPI_APS256)
