...
#define DT_DRV_COMPAT mspi_aps6408l
...
LOG_MODULE_REGISTER(memc_mspi_aps6408l, CONFIG_MEMC_LOG_LEVEL);

#define APM_VENDOR_ID                  0xD

#define APS6408L_SYNC_WRITE              0x80
#define APS6408L_SYNC_READ               0x00
#define APS6408L_LINEAR_BURST_WRITE      0xA0
#define APS6408L_LINEAR_BURST_READ       0x20
#define APS6408L_GLOBAL_RESET            0xFF
#define APS6408L_WRITE_REGISTER          0xC0
#define APS6408L_READ_REGISTER           0x40

struct memc_mspi_aps6408l_config {
	uint32_t                       port;
	uint32_t                       mem_size;
...
struct memc_mspi_aps6408l_data {
	struct memc_mspi_aps6408l_reg    regs;
...
static int memc_mspi_aps6408l_command_write(const struct device *psram, uint8_t cmd,
					  uint32_t addr, uint8_t *wdata, uint32_t length)
{
	const struct memc_mspi_aps6408l_config *cfg = psram->config;
	struct memc_mspi_aps6408l_data *data = psram->data;
	int ret;
...
static int memc_mspi_aps6408l_command_read(const struct device *psram, uint8_t cmd,
...
static int memc_mspi_aps6408l_enter_command_mode(const struct device *psram)
{
	const struct memc_mspi_aps6408l_config *cfg = psram->config;
	struct memc_mspi_aps6408l_data *data = psram->data;
	int ret;
	...
	ret = memc_mspi_aps6408l_command_write(psram, APS6408L_WRITE_REGISTER, 8, &data->regs.MR8, 1);
	if (ret) {
...
static int memc_mspi_aps6408l_exit_command_mode(const struct device *psram)
{
	const struct memc_mspi_aps6408l_config *cfg = psram->config;
	struct memc_mspi_aps6408l_data *data = psram->data;
	int ret;
...
static int memc_mspi_aps6408l_reset(const struct device *psram)
{
	struct memc_mspi_aps6408l_data *data = psram->data;
	int ret = 0;
	...
	ret = memc_mspi_aps6408l_command_write(psram, APS6408L_GLOBAL_RESET, 0,
...
static int memc_mspi_aps6408l_get_vendor_id(const struct device *psram)
{
	struct memc_mspi_aps6408l_data *data = psram->data;
	int ret;

	ret = memc_mspi_aps6408l_command_read(psram, APS6408L_READ_REGISTER, 1, &data->regs.MR1, 1);
...
static int memc_mspi_aps6408l_get_rlc(uint8_t rx_dummy, enum memc_mspi_aps6408l_rlc *rlc)
{
	switch (rx_dummy) {
	case 4:
		*rlc = MEMC_MSPI_APS6408L_RLC_4;
		break;
	case 5:
		*rlc = MEMC_MSPI_APS6408L_RLC_5;
		break;
	case 6:
		*rlc = MEMC_MSPI_APS6408L_RLC_6;
		break;
	case 7:
		*rlc = MEMC_MSPI_APS6408L_RLC_7;
		break;
	case 8:
		*rlc = MEMC_MSPI_APS6408L_RLC_8;
		break;
	case 9:
		*rlc = MEMC_MSPI_APS6408L_RLC_9;
		break;
	default:
		return -EINVAL;
	}
...
static int memc_mspi_aps6408l_get_wlc(uint8_t tx_dummy, enum memc_mspi_aps6408l_wlc *wlc)
{
	switch (tx_dummy) {
	case 5:
		*wlc = MEMC_MSPI_APS6408L_WLC_5;
		break;
	case 6:
		*wlc = MEMC_MSPI_APS6408L_WLC_6;
		break;
	case 7:
		*wlc = MEMC_MSPI_APS6408L_WLC_7;
		break;
	case 8:
		*wlc = MEMC_MSPI_APS6408L_WLC_8;
		break;
	case 9:
		*wlc = MEMC_MSPI_APS6408L_WLC_9;
		break;
	case 10:
		*wlc = MEMC_MSPI_APS6408L_WLC_10;
		break;
	default:
		return -EINVAL;
	}
...
#if CONFIG_PM_DEVICE
static int memc_mspi_aps6408l_half_sleep_enter(const struct device *psram)
{
	const struct memc_mspi_aps6408l_config *cfg = psram->config;
	struct memc_mspi_aps6408l_data *data = psram->data;
	...
	ret = memc_mspi_aps6408l_enter_command_mode(psram);
	if (ret) {
		return ret;
	}

	data->regs.MR6 = 0xF0;
	ret = memc_mspi_aps6408l_command_write(psram, APS6408L_WRITE_REGISTER, 6, &data->regs.MR6, 1);
	if (ret) {
...
static int memc_mspi_aps6408l_half_sleep_exit(const struct device *psram)
{
	const struct memc_mspi_aps6408l_config *cfg = psram->config;
	struct memc_mspi_aps6408l_data *data = psram->data;
	int ret = 0;

	LOG_DBG("Waking up aps6408l from half sleep/%u", __LINE__);
	ret = memc_mspi_aps6408l_command_write(psram, 0, 0, (uint8_t *)&data->dummy, 2);
	if (ret) {
	...
	ret = memc_mspi_aps6408l_exit_command_mode(psram);
	if (ret) {
		return ret;
	}
...
static int memc_mspi_aps6408l_pm_action(const struct device *psram, enum pm_device_action action)
{
	struct memc_mspi_aps6408l_data *data = psram->data;
	...
	case PM_DEVICE_ACTION_RESUME:
		if (data->regs.MR1_b.ULP) {
			acquire(psram);
			memc_mspi_aps6408l_half_sleep_exit(psram);
		}
		break;

	case PM_DEVICE_ACTION_SUSPEND:
		if (data->regs.MR1_b.ULP) {
			memc_mspi_aps6408l_half_sleep_enter(psram);
			release(psram);
		}
		break;
...
static int memc_mspi_aps6408l_init(const struct device *psram)
{
	const struct memc_mspi_aps6408l_config *cfg = psram->config;
	struct memc_mspi_aps6408l_data *data = psram->data;
	...
	data->dev_cfg = cfg->octal_cfg;
printk("APS6408L_READ_REGISTER \n");
	if (memc_mspi_aps6408l_reset(psram)) {
		LOG_ERR("Could not reset pSRAM/%u", __LINE__);
		return -EIO;
	}

	if (memc_mspi_aps6408l_get_vendor_id(psram)) {
		LOG_ERR("Could not read vendor id/%u", __LINE__);
		return -EIO;
	}

	if (memc_mspi_aps6408l_command_read(psram, APS6408L_READ_REGISTER, 0, &data->regs.MR0, 1)) {
		LOG_ERR("Could not read MR0 register/%u", __LINE__);
		return -EIO;
	}

	if (memc_mspi_aps6408l_command_read(psram, APS6408L_READ_REGISTER, 2, &data->regs.MR2, 1)) {
		LOG_ERR("Could not read MR2 register/%u", __LINE__);
		return -EIO;
	}

	if (memc_mspi_aps6408l_command_read(psram, APS6408L_READ_REGISTER, 3, &data->regs.MR3, 1)) {
		LOG_ERR("Could not read MR3 register/%u", __LINE__);
		return -EIO;
	}

	if (memc_mspi_aps6408l_command_read(psram, APS6408L_READ_REGISTER, 4, &data->regs.MR4, 1)) {
		LOG_ERR("Could not read MR4 register/%u", __LINE__);
		return -EIO;
	}

	if (memc_mspi_aps6408l_command_read(psram, APS6408L_READ_REGISTER, 6, &data->regs.MR6, 1)) {
		LOG_ERR("Could not read MR2 register/%u", __LINE__);
		return -EIO;
	}

	if (memc_mspi_aps6408l_command_read(psram, APS6408L_READ_REGISTER, 8, &data->regs.MR8, 1)) {
		LOG_ERR("Could not read MR8 register/%u", __LINE__);
		return -EIO;
	}

	enum memc_mspi_aps6408l_rlc rlc;

	if (memc_mspi_aps6408l_get_rlc(cfg->tar_dev_cfg.rx_dummy, &rlc)) {
		LOG_ERR("rx_dummy:%d not supported/%u", cfg->tar_dev_cfg.rx_dummy, __LINE__);
		return -EIO;
	}
	data->regs.MR0_b.RLC = rlc;

	enum memc_mspi_aps6408l_wlc wlc;

	if (memc_mspi_aps6408l_get_wlc(cfg->tar_dev_cfg.tx_dummy, &wlc)) {
	...
	if (memc_mspi_aps6408l_command_write(psram, APS6408L_WRITE_REGISTER, 0, &data->regs.MR0, 1)) {
		LOG_ERR("Could not write MR0 register/%u", __LINE__);
		return -EIO;
	}

	if (memc_mspi_aps6408l_command_write(psram, APS6408L_WRITE_REGISTER, 4, &data->regs.MR4, 1)) {
	...
	if (cfg->tar_dev_cfg.io_mode == MSPI_IO_MODE_HEX_8_8_16) {
		...
		if (memc_mspi_aps6408l_command_write(psram, APS6408L_WRITE_REGISTER, 8,
						   &data->regs.MR8, 1)) {
...
#define MSPI_DEVICE_CONFIG_OCTAL(n)                                                               \
	{                                                                                         \
		...
		.rx_dummy           = MEMC_MSPI_APS6408L_RX_DUMMY_DEFAULT,                          \
		.tx_dummy           = MEMC_MSPI_APS6408L_TX_DUMMY_DEFAULT,                          \
		.read_cmd           = APS6408L_LINEAR_BURST_READ,                                   \
		.write_cmd          = APS6408L_LINEAR_BURST_WRITE,                                  \
		.cmd_length         = MEMC_MSPI_APS6408L_CMD_LENGTH_DEFAULT,                        \
		.addr_length        = MEMC_MSPI_APS6408L_ADDR_LENGTH_DEFAULT,                       \
		.mem_boundary       = 1024,                                                       \
		.time_to_break      = 4,                                                          \
	}
...
#define MEMC_MSPI_APS6408L(n)                                                                       \
	static const struct memc_mspi_aps6408l_config                                               \
	memc_mspi_aps6408l_config_##n = {                                                           \
		.port               = MSPI_PORT(n),                                               \
	...
	static struct memc_mspi_aps6408l_data                                                       \
		memc_mspi_aps6408l_data_##n = {                                                     \
		.lock  = Z_SEM_INITIALIZER(memc_mspi_aps6408l_data_##n.lock, 0, 1),                 \
		.dummy = 0,                                                                       \
	};                                                                                        \
	PM_DEVICE_DT_INST_DEFINE(n, memc_mspi_aps6408l_pm_action);                                  \
	DEVICE_DT_INST_DEFINE(n,                                                                  \
			      memc_mspi_aps6408l_init,                                              \
...
DT_INST_FOREACH_STATUS_OKAY(MEMC_MSPI_APS6408L)
