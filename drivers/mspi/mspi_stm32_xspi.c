...
static inline int mspi_stm32_xspi_context_lock(struct mspi_context *ctx,
 ...
 */
static inline bool mspi_stm32_xspi_is_inp(const struct device *controller)
{
	struct mspi_stm32_data *dev_data = controller->data;
...
static int read_write_in_indirect_mode(const struct device *dev, struct mspi_xfer_packet *packet,
				       uint8_t access_mode)
{
	int ret = 0;
	struct mspi_stm32_data *dev_data = dev->data;
	...
	if (packet->dir == MSPI_RX) {
		...
		case MSPI_ACCESS_DMA:
			uint8_t *dma_buf =
				k_aligned_alloc(CONFIG_MSPI_BUFFER_ALIGNMENT, packet->num_bytes);
				  ...
				  uint8_t access_mode)
{
	...
	if (dev_data->xip_cfg.enable) {
		...
		if ((packet->cmd == SPI_NOR_CMD_WREN) || (packet->cmd == SPI_NOR_OCMD_WREN) ||
			...
			if (mspi_stm32_xspi_is_memorymap(dev)) {
				if (mspi_stm32_xspi_memmap_off(dev)) {
					LOG_ERR("Failed to abort memory-mapped access");
					return -EIO;
				}
			}
...
indirect:
	...
	if ((ret != HAL_OK) || (packet->num_bytes == 0)) {
		pm_policy_state_lock_put(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
		(void)pm_device_runtime_put(dev);
		if (ret != HAL_OK) {
...
static int mspi_stm32_xspi_mem_ready(const struct device *dev, uint8_t cfg_mode, uint8_t cfg_rate)
{
	...
	return mspi_stm32_xspi_wait_auto_polling(dev, SPI_NOR_MEM_RDY_MATCH,
						 SPI_NOR_MEM_RDY_MASK,
						 HAL_XSPI_TIMEOUT_DEFAULT_VALUE);
}
...
static void mspi_stm32_xspi_dma_callback(const struct device *dev, void *arg, uint32_t channel,
					 int status)
{
 ...
 */
static inline int mspi_stm32_xspi_verify_device(const struct device *controller,
				      ...
				      const struct mspi_dev_cfg *dev_cfg)
{
	...
	if (mspi_stm32_xspi_dev_cfg_save(controller, param_mask, dev_cfg)) {
...
static int mspi_stm32_xspi_dma_init(DMA_HandleTypeDef *hdma, struct stream *dma_stream)
{
	int ret;
	/*
	 * DMA configuration
...
static int mspi_dma_setup(const struct mspi_stm32_conf *dev_cfg, struct mspi_stm32_data *dev_data)
{
	...
	if (mspi_stm32_xspi_dma_init(&hdma_tx, &dev_data->dma_tx) != 0) {
		LOG_ERR("XSPI DMA Tx init failed");
		return -EIO;
	}
	__HAL_LINKDMA(&dev_data->hmspi.xspi, hdmatx, hdma_tx);

	if (mspi_stm32_xspi_dma_init(&hdma_rx, &dev_data->dma_rx) != 0) {
