...
//#define MSPI_STM32_NODE DT_INST_PARENT(0)
#define MSPI_NOR_STM32_NODE DT_INST_CHILD(0)
...
static int mspi_stm32_mem_erased(const struct device *dev)
{
	...
	XSPI_RegularCmdTypeDef s_command = flash_mspi_stm32_prepare_cmd(nor_mode, nor_rate);
 ...
 * and nor_rate transfer STR/DTR MSPI_DATA_RATE_SINGLE or MSPI_DATA_RATE_DUAL
 */
static int mspi_stm32_mem_ready(const struct device *dev, uint8_t nor_mode,
		uint8_t nor_rate)
{
	struct mspi_stm32_data *dev_data = dev->data;
	...
	return flash_mspi_stm32_wait_auto_polling(dev, &s_config, HAL_XSPI_TIMEOUT_DEFAULT_VALUE);
}
 ...
 */
static int mspi_pio_prepare_cmd(const struct device *controller,
			    XSPI_RegularCmdTypeDef *cmd_tmp)
{
	const struct mspi_stm32_config *dev_cfg = controller->config;
	struct mspi_stm32_data *dev_data = controller->data;
	const struct mspi_xfer *xfer = &dev_data->ctx.xfer;
	int ret = 0;

	cmd_tmp->OperationType = HAL_XSPI_OPTYPE_COMMON_CFG;
	cmd_tmp->InstructionWidth = ((dev_data->dev_cfg.io_mode == MSPI_IO_MODE_OCTAL)
			? HAL_XSPI_INSTRUCTION_16_BITS
			: HAL_XSPI_INSTRUCTION_8_BITS);
	cmd_tmp->InstructionDTRMode = ((dev_data->dev_cfg.io_mode == MSPI_DATA_RATE_DUAL)
				? HAL_XSPI_INSTRUCTION_DTR_ENABLE
				: HAL_XSPI_INSTRUCTION_DTR_DISABLE);
	cmd_tmp->AlternateBytesMode = HAL_XSPI_ALT_BYTES_NONE;
	cmd_tmp->DataDTRMode = ((dev_data->dev_cfg.data_rate == MSPI_DATA_RATE_DUAL)
			? HAL_XSPI_DATA_DTR_ENABLE
			: HAL_XSPI_DATA_DTR_DISABLE);
	cmd_tmp->DQSMode = ((dev_data->dev_cfg.data_rate == MSPI_DATA_RATE_DUAL)
			? HAL_XSPI_DQS_ENABLE
			: HAL_XSPI_DQS_DISABLE);
	cmd_tmp->SIOOMode = HAL_XSPI_SIOO_INST_EVERY_CMD;

	if (xfer->cmd_length > AM_HAL_MSPI_INSTR_2_BYTE + 1) {
		LOG_ERR("invalid cmd_length %d", xfer->cmd_length);
		return -ENOTSUP;
	}
	if (xfer->cmd_length != 0) {
		stm32_hal_mspi_instr_e eInstrCfg = xfer->cmd_length - 1;

		ret = stm32_hal_mspi_control(dev_data->mspiHandle, AM_HAL_MSPI_REQ_ISIZE_SET, &eInstrCfg);
		if (ret) {
			LOG_ERR("failed to configure cmd_length.");
			return -EHOSTDOWN;
		}
		dev_data->hal_dev_cfg.eInstrCfg = eInstrCfg;
	}
	dev_data->dev_cfg.cmd_length = xfer->cmd_length;

	if (xfer->addr_length > 4U) {
		LOG_ERR("%u, invalid addr_length.", xfer->addr_length);
		return -ENOTSUP;
	}
	if (xfer->addr_length != 0) {
		uint32_t AddrCfg = xfer->addr_length - 1;

		ret = stm32_hal_mspi_control(dev_data->mspiHandle, AM_HAL_MSPI_REQ_ASIZE_SET, &AddrCfg);
		if (ret) {
			LOG_ERR("failed to configure addr_length.");
			return -EHOSTDOWN;
		}
		dev_data->hal_dev_cfg.eAddrCfg = AddrCfg;
	}
	dev_data->dev_cfg.addr_length = xfer->addr_length;
			       ...
			       struct mspi_callback_context *cb_ctx)
{
	...
	if (cfg_flag) {
		if (cfg_flag == 1) {
			ret = mspi_pio_prepare(controller, &transf);
			if (ret) {
				goto pio_err;
			}
		} else {
	...
	if (!ctx->xfer.async) {
		...
		while (ctx->packets_left > 0) {
			...
			transf.eDirection         = packet->dir;
			transf.ui16DeviceInstr    = (uint16_t)packet->cmd;
			transf.ui32DeviceAddr     = packet->address;
			transf.ui32NumBytes       = packet->num_bytes;
			transf.pui32Buffer        = (uint32_t *)packet->data_buf;

			ret = stm32_hal_mspi_blocking_transfer(dev_data->hmspi, &transf,
							    MSPI_TIMEOUT_US);
			ctx->packets_left--;
			if (ret) {
				ret = -EIO;
		...
		ret = stm32_hal_mspi_interrupt_enable(dev_data->hmspi, AM_HAL_MSPI_INT_DMACMP);
		if (ret) {
		...
		while (ctx->packets_left > 0) {
			...
			stm32_hal_mspi_callback_t callback = NULL;

			if (packet->cb_mask == MSPI_BUS_XFER_COMPLETE_CB) {
				callback = (stm32_hal_mspi_callback_t)ctx->callback;
			}

			ret = stm32_hal_mspi_nonblocking_transfer(dev_data->hmspi, &transf, MSPI_PIO,
			       ...
			       struct mspi_callback_context *cb_ctx)
{
	...
	while (ctx->packets_left > 0) {
		...
		packet                     = &ctx->xfer.packets[packet_idx];
		transf.ui8Priority          = ctx->xfer.priority;
		transf.eDirection           = packet->dir;
		transf.ui32TransferCount    = packet->num_bytes;
		transf.ui32DeviceAddress    = packet->address;
		transf.ui32SRAMAddress      = (uint32_t)packet->data_buf;
		transf.ui32PauseCondition   = 0;
		transf.ui32StatusSetClr     = 0;
		...
		if (ctx->xfer.async) {
			...
			ret = stm32_hal_mspi_nonblocking_transfer(dev_data->hmspi, &transf, MSPI_DMA,
							       callback, (void *)ctx->callback_ctx);
		} else {
			ret = stm32_hal_mspi_nonblocking_transfer(dev_data->hmspi, &transf, MSPI_DMA,
							       hal_mspi_callback,
 ...
 */
static int mspi_stm32_config(const struct device *dev)
{
	const struct mspi_stm32_conf *dev_cfg = dev->config;
	struct mspi_stm32_data *dev_data = dev->data;
	...
	if (dev_cfg->mspicfg.op_mode != MSPI_OP_MODE_CONTROLLER) {
		LOG_ERR("only support MSPI controller mode.");
		return -ENOTSUP;
	}

	/* Check the max possible freq. */
	if (dev_cfg->mspicfg.max_freq > MSPI_MAX_FREQ) {
		LOG_ERR("max_freq %d too large.", dev_cfg->mspicfg.max_freq);
		return -ENOTSUP;
	}

	if (dev_cfg->mspicfg.duplex != MSPI_HALF_DUPLEX) {
	...
	for (; prescaler <= MSPI_STM32_CLOCK_PRESCALER_MAX; prescaler++) {
		 dev_data->cfg.freq = MSPI_STM32_CLOCK_COMPUTE(ahb_clock_freq, prescaler);
	...
	ret = mspi_stm32_read_jedec_id(dev);
	if (ret != 0) {
...
#endif /* CONFIG_FLASH_JESD216_API */
#endif
	if (mspi_stm32_config_mem(dev) != 0) {
...
static void mspi_stm32_irq_config_func(const struct device *dev);

static const struct stm32_pclken pclken[] = STM32_DT_CLOCKS(MSPI_STM32_NODE);

PINCTRL_DT_DEFINE(MSPI_STM32_NODE);

static const struct mspi_stm32_conf mspi_stm32_dev_conf = {
	.pclken = pclken,
	.pclk_len = DT_NUM_CLOCKS(MSPI_STM32_NODE),
	.irq_config = mspi_stm32_irq_config_func,
	.mspicfg = MSPI_CONFIG(0),
	.pcfg = PINCTRL_DT_DEV_CONFIG_GET(MSPI_STM32_NODE),
#if MSPI_STM32_RESET_GPIO
...
static struct mspi_stm32_data mspi_stm32_dev_data = {
	.hmspi = {
		.Instance = (XSPI_TypeDef *)DT_REG_ADDR(MSPI_STM32_NODE),
		.Init = {
			.FifoThresholdByte = MSPI_STM32_FIFO_THRESHOLD,
			.SampleShifting = (DT_PROP(MSPI_STM32_NODE, ssht_enable)
	...
	.lock = Z_MUTEX_INITIALIZER(mspi_stm32_data.lock),
	.dev_cfg = MSPI_DEVICE_CONFIG_DT_INST(0),
	.xip_cfg = {0},
	.scramble_cfg = {0},
	.timing_cfg = {0},
	.cbs = {0},
	.ctx.lock = Z_SEM_INITIALIZER(mspi_stm32_data.ctx.lock, 0, 1),
	.ctx.callback = 0,
	.ctx.callback_ctx = 0,
