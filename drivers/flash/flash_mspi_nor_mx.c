...
#define MSPI_DEVICE_CONFIG_SERIAL(n)                                                               \
	{                                                                                          \
		...
		.cpp = MSPI_CPP_MODE_0, .endian = MSPI_XFER_LITTLE_ENDIAN,                         \
		.ce_polarity = MSPI_CE_ACTIVE_LOW, .dqs_enable = false,                            \
		...
		.write_cmd = DT_INST_PROP_OR(n, write_command, SPI_NOR_CMD_PP), .cmd_length = 1,   \
		.addr_length = 4, .mem_boundary = 0, .time_to_break = 0,                           \
	}
