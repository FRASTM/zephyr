...
LOG_MODULE_REGISTER(flash_mspi_stm32, CONFIG_FLASH_LOG_LEVEL);
...
#define REG_MSPI_BASEADDR  OCTOSPI1_R_BASE_NS

#define FLASH_MSPI_STM32_TIMEOUT HAL_XSPI_TIMEOUT_DEFAULT_VALUE

enum stm32_dummy_clock {
	STM32_DC_8,
	STM32_DC_10,
	STM32_DC_12,
	STM32_DC_14,
...
struct flash_mspi_stm32_config {
	uint32_t                            port;
	uint32_t                            mem_size;
...
static bool flash_mspi_stm32_address_is_valid(const struct device *dev, off_t addr,
				  size_t size)
{
	const struct flash_mspi_stm32_config *dev_cfg = dev->config;
	uint32_t flash_size = dev_cfg->mem_size;

	return (addr >= 0) && ((uint64_t)addr + (uint64_t)size <= flash_size);
}

static int flash_stm32_get_dummy_clk(uint8_t rxdummy, uint32_t *dummy_clk)
{
	switch (rxdummy) {
	case 8:
		*dummy_clk = STM32_DC_8;
		break;
	case 10:
		*dummy_clk = STM32_DC_10;
		break;
	case 12:
		*dummy_clk = STM32_DC_12;
		break;
	case 14:
		*dummy_clk = STM32_DC_14;
		break;
	case 16:
		*dummy_clk = STM32_DC_16;
		break;
	case 18:
		*dummy_clk = STM32_DC_18;
		break;
	case 20:
		*dummy_clk = STM32_DC_20;
		break;
	case 22:
		*dummy_clk = STM32_DC_22;
		break;
	default:
		return 1;
	}
...
static int flash_mspi_stm32_command_write(const struct device *flash, uint8_t cmd, uint32_t addr,
					    uint16_t addr_len, uint32_t tx_dummy, uint8_t *wdata,
					    uint32_t length)
{
	const struct flash_mspi_stm32_config *cfg = flash->config;
	struct flash_mspi_stm32_data *data = flash->data;
	int ret;
...
static int flash_mspi_stm32_command_read(const struct device *flash, uint8_t cmd, uint32_t addr,
...
static int flash_mspi_stm32_write_enable(const struct device *flash)
{
	int ret;
	...
	ret = flash_mspi_stm32_command_write(flash, SPI_NOR_CMD_WREN, 0, 0, 0, NULL, 0);

	/* New command to Configure automatic polling mode to wait for write enabling */
	ret = flash_mspi_stm32_command_write(flash, SPI_NOR_CMD_RDSR, 0, 0, 0, NULL, 0);
...
static int flash_mspi_stm32_write_disable(const struct device *flash)
{
	int ret;

	LOG_DBG("Disabling write");
	ret = flash_mspi_stm32_command_write(flash, SPI_NOR_CMD_WRDI, 0, 0, 0, NULL, 0);

	return ret;
}

static int flash_mspi_stm32_reset(const struct device *flash)
{
	int ret;

	/* TODO : reset by gpio pin :set pi, wait duration, release pin */

	ret = flash_mspi_stm32_write_enable(flash);
	if (ret) {
		return ret;
	}

	/* Send Reset enable then reset Mem */
	ret = flash_mspi_stm32_command_write(flash, SPI_NOR_CMD_RESET_EN, 0, 0, 0, NULL, 0);
	if (ret) {
		return ret;
	}
	ret = flash_mspi_stm32_command_write(flash, SPI_NOR_CMD_RESET_MEM, 0, 0, 0, NULL, 0);
	if (ret) {
		return ret;
	}

	ret = flash_mspi_stm32_write_disable(flash);
	if (ret) {
		return ret;
	}
...
static int flash_mspi_stm32_get_vendor_id(const struct device *flash, uint8_t *vendor_id)
{
	struct flash_mspi_stm32_data *data = flash->data;
	uint8_t buffer[11];
	int ret;
	...
	ret = flash_mspi_stm32_command_read(flash, SPI_NOR_CMD_RDID, 0, 0, 0, buffer, 11);
	*vendor_id = buffer[7];
...
static int flash_mspi_stm32_unprotect_sector(const struct device *flash, off_t addr)
{
	int ret;

	LOG_DBG("unprotect sector at 0x%08zx", (ssize_t)addr);

	ret = flash_mspi_stm32_command_write(flash, 0x39, addr, 4, 0, NULL, 0);

	return ret;
}

static int flash_mspi_stm32_erase_sector(const struct device *flash, off_t addr)
{
	int ret;

	LOG_DBG("Erasing sector at 0x%08zx", (ssize_t)addr);

	ret = flash_mspi_stm32_command_write(flash, SPI_NOR_CMD_SE, addr, 4, 0, NULL, 0);

	return ret;
}

static int flash_mspi_stm32_erase_block(const struct device *flash, off_t addr)
{
	int ret;

	LOG_DBG("Erasing block at 0x%08zx", (ssize_t)addr);

	ret = flash_mspi_stm32_command_write(flash, SPI_NOR_CMD_BE, addr, 4, 0, NULL, 0);

	return ret;
}

static int flash_mspi_stm32_erase_chip(const struct device *flash)
{
	int ret;

	LOG_DBG("Erasing chip");

	ret = flash_mspi_stm32_command_write(flash, SPI_NOR_CMD_CE, 0, 0, 0, NULL, 0);

	return ret;
}

static int flash_mspi_stm32_page_program(const struct device *flash, off_t offset, void *wdata,
					   size_t len)
{
	const struct flash_mspi_stm32_config *cfg = flash->config;
	struct flash_mspi_stm32_data *data = flash->data;
	...
	data->trans.packets           = &data->packet;
	data->trans.num_packet        = 1;
	data->trans.timeout           = FLASH_MSPI_STM32_TIMEOUT;
 ...
 */
static int flash_mspi_stm32_mem_ready(const struct device *flash)
{
	const struct flash_mspi_stm32_config *cfg = flash->config;
	struct flash_mspi_stm32_data *data = flash->data;
...
//		TIMING_CFG_SET_RX_DUMMY(&data->timing_cfg, 4);
	...
	do {
		...
		ret = flash_mspi_stm32_command_read(flash, instruction, 0, 0, rx_dummy,
						      (uint8_t *)&status, 1);
		if (ret) {
...
static int flash_mspi_stm32_read(const struct device *flash, off_t offset, void *rdata,
				   size_t size)
{
	const struct flash_mspi_stm32_config *cfg = flash->config;
	struct flash_mspi_stm32_data *data = flash->data;
	...
	if (!flash_mspi_stm32_address_is_valid(flash, offset, size)) {
	...
	if (cfg->tar_xip_cfg.enable) {
		...
		uintptr_t mmap_addr = REG_MSPI_BASEADDR + offset;
...
static int flash_mspi_stm32_write(const struct device *flash, off_t offset, const void *wdata,
				    size_t size)
{
	const struct flash_mspi_stm32_config *cfg = flash->config;
	struct flash_mspi_stm32_data *data = flash->data;
	...
	while (size) {
		...
		ret = flash_mspi_stm32_page_program(flash, offset, src, i);
		if (ret) {
			goto write_end;
		}

		ret = flash_mspi_stm32_mem_ready(flash);
		if (ret) {
			goto write_end;
		}
 ...
 *   set addr = 0
 */
static int flash_mspi_stm32_erase(const struct device *flash, off_t offset, size_t size)
{
	const struct flash_mspi_stm32_config *cfg = flash->config;
	struct flash_mspi_stm32_data *data = flash->data;
	int ret = 0;
	...
	if ((offset == 0) && (size == cfg->mem_size)) {
		...
		ret = flash_mspi_stm32_erase_chip(flash);
		if (ret) {
			goto erase_end;
		}
		...
		for (i = 0; i < num_blocks; i++) {
			...
			ret = flash_mspi_stm32_unprotect_sector(flash, offset);
			if (ret) {
				goto erase_end;
			}
			...
			ret = flash_mspi_stm32_erase_block(flash, offset);
			if (ret) {
				goto erase_end;
			}
		...
		for (i = 0; i < num_sectors; i++) {
			...
			ret = flash_mspi_stm32_erase_sector(flash, offset);
			if (ret) {
				goto erase_end;
			}
...
static const struct flash_parameters *flash_mspi_stm32_get_parameters(const struct device *flash)
{
	const struct flash_mspi_stm32_config *cfg = flash->config;
...
static void flash_mspi_stm32_pages_layout(const struct device *flash,
...
static int flash_mspi_stm32_init(const struct device *flash)
{
	const struct flash_mspi_stm32_config *cfg = flash->config;
	struct flash_mspi_stm32_data *data = flash->data;
	uint8_t vendor_id;
	...
	if (flash_mspi_stm32_reset(flash)) {
		LOG_ERR("Could not reset Flash/%u", __LINE__);
		return -EIO;
	}

	if (flash_mspi_stm32_get_vendor_id(flash, &vendor_id)) {
		LOG_ERR("Could not read vendor id/%u", __LINE__);
		return -EIO;
	}
	LOG_DBG("Vendor id: 0x%0x", vendor_id);

	if (flash_mspi_stm32_write_enable(flash)) {
		return -EIO;
	}

	if (flash_mspi_stm32_command_write(flash, SPI_NOR_CMD_RD_CFGREG2, 0x3, 1, 0, (uint8_t *)&CRB3, 1)) {
		return -EIO;
	}
	...
	if (flash_mspi_stm32_command_write(flash, cmd, 0, 0, 0, NULL, 0)) {
		return -EIO;
	}
...
static int flash_mspi_stm32_read_sfdp(const struct device *flash, off_t addr, void *rdata,
					size_t size)
{
	const struct flash_mspi_stm32_config *cfg = flash->config;
	struct flash_mspi_stm32_data *data = flash->data;
...
static int flash_mspi_stm32_read_jedec_id(const struct device *flash, uint8_t *id)
{
	struct flash_mspi_stm32_data *data = flash->data;
...
static int flash_mspi_stm32_pm_action(const struct device *flash, enum pm_device_action action)
{
	switch (action) {
...
static const struct flash_driver_api flash_mspi_stm32_api = {
	.erase = flash_mspi_stm32_erase,
	.write = flash_mspi_stm32_write,
	.read = flash_mspi_stm32_read,
	.get_parameters = flash_mspi_stm32_get_parameters,
#if defined(CONFIG_FLASH_PAGE_LAYOUT)
	.page_layout = flash_mspi_stm32_pages_layout,
#endif
#if defined(CONFIG_FLASH_JESD216_API)
	.sfdp_read = flash_mspi_stm32_read_sfdp,
	.read_jedec_id = flash_mspi_stm32_read_jedec_id,
#endif
};
...
#if 0
#define FLASH_MSPI_STM32(n)									\
	static const struct flash_mspi_stm32_config flash_mspi_stm32_config_##n = { 		\
	...
	static struct flash_mspi_stm32_data flash_mspi_stm32_data_##n = {			\
		.lock = Z_SEM_INITIALIZER(flash_mspi_stm32_data_##n.lock, 0, 1), 		\
	};											\
	PM_DEVICE_DT_INST_DEFINE(n, flash_mspi_stm32_pm_action);				\
	DEVICE_DT_INST_DEFINE(n,								\
			      flash_mspi_stm32_init,                                            \
			      PM_DEVICE_DT_INST_GET(n),						\
			      &flash_mspi_stm32_data_##n,					\
			      &flash_mspi_stm32_config_##n,					\
			      POST_KERNEL,							\
			      CONFIG_FLASH_INIT_PRIORITY,					\
			      &flash_mspi_stm32_api);

DT_INST_FOREACH_STATUS_OKAY(FLASH_MSPI_STM32)
#else
static const struct flash_mspi_stm32_config flash_mspi_stm32_cfg = {
...
static struct flash_mspi_stm32_data flash_mspi_stm32_dev_data = {
		.lock = Z_SEM_INITIALIZER(flash_mspi_stm32_dev_data.lock, 0, 1),
...
DEVICE_DT_INST_DEFINE(0,
		...
		&flash_mspi_stm32_dev_data,
		&flash_mspi_stm32_cfg,
		POST_KERNEL,
		CONFIG_FLASH_INIT_PRIORITY,
		&flash_mspi_stm32_api);
#endif
