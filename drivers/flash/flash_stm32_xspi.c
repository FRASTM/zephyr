...
printk("\nOSPI wr in OPI/STR mode is for 32bit address only");
		return -EIO;
	}
	...
	if (hal_ret != HAL_OK) {
		LOG_ERR("%d: Failed to send OSPI instruction", hal_ret);
printk("\n%d: Failed to send OSPI instruction", hal_ret);
		return -EIO;
	}

#if STM32_OSPI_USE_DMA
	hal_ret = HAL_OSPI_Transmit_DMA(&dev_data->hospi, (uint8_t *)data);
printk("\n%d: Failed to write DMA data (%d)", hal_ret, dev_data->hospi.ErrorCode);
#else
				  ...
				  const void *data, size_t size)
{
	...
	if (!ospi_address_is_valid(dev, addr, size)) {
		LOG_ERR("Error: address or size exceeds expected values: "
			"addr 0x%lx, size %zu", (long)addr, size);
printk("\n Error: address or size exceeds expected values: "
			"addr 0x%lx, size %zu", (long)addr, size);
		return -EINVAL;
	}
...
hdma.Init.DestBurstLength = 1;
	...
	/* This is specific to the stm32h5_hal with XSPI_HandleTypeDef */
...
#define OSPI_DMA_CHANNEL_INIT(index, dir, dir_cap, src_dev, dest_dev)	\
...
#define OSPI_DMA_CHANNEL(id, dir, DIR, src, dest)			\
	.dma_##dir = {							\
		COND_CODE_1(DT_INST_DMAS_HAS_NAME(id, dir),		\
			(OSPI_DMA_CHANNEL_INIT(id, dir, DIR, src, dest)),\
			(NULL))						\
		},
...
#endif /* jedec_id */
#if defined(CONFIG_SOC_SERIES_STM32H5X)
	OSPI_DMA_CHANNEL(STM32_OSPI_NODE, rx, RX, PERIPHERAL, MEMORY)
	OSPI_DMA_CHANNEL(STM32_OSPI_NODE, tx, TX, MEMORY, PERIPHERAL)
#else
