 ...
 * **************************************************************************
 */
#define DT_DRV_COMPAT st_stm32_mspi_nor
...
LOG_MODULE_REGISTER(flash_stm32_mspi, CONFIG_FLASH_LOG_LEVEL);

#define STM32_MSPI_NODE DT_INST_PARENT(0)

#define DT_MSPI_IO_PORT_PROP_OR(prop, default_value)					\
	COND_CODE_1(DT_NODE_HAS_PROP(STM32_MSPI_NODE, prop),				\
		    (_CONCAT(HAL_XSPIM_, DT_STRING_TOKEN(STM32_MSPI_NODE, prop))),	\
		    ((default_value)))

/* Get the base address of the flash from the DTS node */
#define STM32_MSPI_BASE_ADDRESS DT_INST_REG_ADDR(0)

#define STM32_MSPI_RESET_GPIO DT_INST_NODE_HAS_PROP(0, reset_gpios)

#define STM32_MSPI_DLYB_BYPASSED DT_PROP(STM32_MSPI_NODE, dlyb_bypass)

#define STM32_MSPI_USE_DMA DT_NODE_HAS_PROP(STM32_MSPI_NODE, dmas)

#if STM32_MSPI_USE_DMA
#include <zephyr/drivers/dma/dma_stm32.h>
#include <zephyr/drivers/dma.h>
#include <stm32_ll_dma.h>
#endif /* STM32_MSPI_USE_DMA */

#include "flash_stm32_mspi.h"

static inline void mspi_lock_thread(const struct device *dev)
{
	struct flash_stm32_mspi_data *dev_data = dev->data;
...
static int mspi_send_cmd(const struct device *dev, XSPI_RegularCmdTypeDef *cmd)
{
	struct flash_stm32_mspi_data *dev_data = dev->data;
...
static int mspi_read_access(const struct device *dev, XSPI_RegularCmdTypeDef *cmd,
...
#if STM32_MSPI_USE_DMA
	hal_ret = HAL_XSPI_Receive_DMA(&dev_data->hmspi, data);
#else
	hal_ret = HAL_XSPI_Receive_IT(&dev_data->hmspi, data);
#endif
...
static int mspi_write_access(const struct device *dev, XSPI_RegularCmdTypeDef *cmd,
			     const uint8_t *data, const size_t size)
{
	...
	if ((dev_data->cfg.io_mode == MSPI_IO_MODE_OCTAL) &&
...
#if STM32_MSPI_USE_DMA
	hal_ret = HAL_XSPI_Transmit_DMA(&dev_data->hmspi, (uint8_t *)data);
#else
	hal_ret = HAL_XSPI_Transmit_IT(&dev_data->hmspi, (uint8_t *)data);
#endif
 ...
 */
static XSPI_RegularCmdTypeDef mspi_prepare_cmd(const uint8_t transfer_mode,
					       const uint8_t transfer_rate)
{
	XSPI_RegularCmdTypeDef cmd_tmp = {
		.OperationType = HAL_XSPI_OPTYPE_COMMON_CFG,
		.InstructionWidth = ((transfer_mode == MSPI_IO_MODE_OCTAL)
				? HAL_XSPI_INSTRUCTION_16_BITS
				: HAL_XSPI_INSTRUCTION_8_BITS),
		.InstructionDTRMode = ((transfer_rate == MSPI_DATA_RATE_DUAL)
				? HAL_XSPI_INSTRUCTION_DTR_ENABLE
				: HAL_XSPI_INSTRUCTION_DTR_DISABLE),
		.AddressDTRMode = ((transfer_rate == MSPI_DATA_RATE_DUAL)
		...
		.DataDTRMode = ((transfer_rate == MSPI_DATA_RATE_DUAL)
				? HAL_XSPI_DATA_DTR_ENABLE
				: HAL_XSPI_DATA_DTR_DISABLE),
		.DQSMode = (transfer_rate == MSPI_DATA_RATE_DUAL)
				? HAL_XSPI_DQS_ENABLE
...
static uint32_t stm32_mspi_hal_address_size(const struct device *dev)
{
	struct flash_stm32_mspi_data *dev_data = dev->data;
 ...
 * and store in the jedec_id Table of the flash_stm32_mspi_data
 * The JEDEC ID is not given by a DTS property
 */
static int stm32_mspi_read_jedec_id(const struct device *dev)
{
	struct flash_stm32_mspi_data *dev_data = dev->data;
	...
	cmd.AddressWidth = stm32_mspi_hal_address_size(dev);
 ...
 */
static int stm32_mspi_read_sfdp(const struct device *dev, off_t addr,
				void *data,
				size_t size)
{
	struct flash_stm32_mspi_data *dev_data = dev->data;

	XSPI_RegularCmdTypeDef cmd = mspi_prepare_cmd(dev_data->cfg.io_mode,
						      dev_data->cfg.data_rate);
	if (dev_data->cfg.io_mode == MSPI_IO_MODE_OCTAL) {
			  ...
			  size_t size)
{
	...
	if (stm32_mspi_read_sfdp(dev, addr, data, size) == 0) {
...
static bool mspi_address_is_valid(const struct device *dev, off_t addr,
				  size_t size)
{
	const struct flash_stm32_mspi_config *dev_cfg = dev->config;
	size_t flash_size = dev_cfg->flash_size;

	return (addr >= 0) && ((uint64_t)addr + (uint64_t)size <= flash_size);
}

static int stm32_mspi_wait_auto_polling(const struct device *dev,
 ...
 * in nor_mode SPI/OPI MSPI_IO_MODE_SINGLE or MSPI_IO_MODE_OCTAL
 * and nor_rate transfer STR/DTR MSPI_DATA_RATE_SINGLE or MSPI_DATA_RATE_DUAL
 */
static int stm32_mspi_mem_erased(const struct device *dev)
{
	struct flash_stm32_mspi_data *dev_data = dev->data;
	uint8_t nor_mode = dev_data->cfg.io_mode;
	uint8_t nor_rate = dev_data->cfg.data_rate;

	XSPI_AutoPollingTypeDef s_config = {0};
	XSPI_RegularCmdTypeDef s_command = mspi_prepare_cmd(nor_mode, nor_rate);

	/* Configure automatic polling mode command to wait for memory ready */
	if (nor_mode == MSPI_IO_MODE_OCTAL) {
		s_command.Instruction = SPI_NOR_OCMD_RDSR;
		s_command.DummyCycles = (nor_rate == MSPI_DATA_RATE_DUAL)
	...
	s_command.DataLength = ((nor_rate == MSPI_DATA_RATE_DUAL) ? 2U : 1U);
	...
	return stm32_mspi_wait_auto_polling(dev,
			&s_config, STM32_MSPI_BULK_ERASE_MAX_TIME);
}
 ...
 * and nor_rate transfer STR/DTR MSPI_DATA_RATE_SINGLE or MSPI_DATA_RATE_DUAL
 */
static int stm32_mspi_mem_ready(const struct device *dev, uint8_t nor_mode,
		uint8_t nor_rate)
{
	struct flash_stm32_mspi_data *dev_data = dev->data;
	...
	return stm32_mspi_wait_auto_polling(dev, &s_config, HAL_XSPI_TIMEOUT_DEFAULT_VALUE);
}

/* Enables writing to the memory sending a Write Enable and wait it is effective */
static int stm32_mspi_write_enable(const struct device *dev,
...
static int stm32_mspi_write_cfg2reg_dummy(XSPI_HandleTypeDef *hmspi,
					uint8_t nor_mode, uint8_t nor_rate)
{
	...
	s_command.Instruction = (nor_mode == MSPI_IO_MODE_SINGLE)
				? SPI_NOR_CMD_WR_CFGREG2
				: SPI_NOR_OCMD_WR_CFGREG2;
	s_command.Address = SPI_NOR_REG2_ADDR3;
	s_command.DummyCycles = 0U;
	s_command.DataLength = (nor_mode == MSPI_IO_MODE_SINGLE) ? 1U
			: ((nor_rate == MSPI_DATA_RATE_DUAL) ? 2U : 1U);
...
static int stm32_mspi_write_cfg2reg_io(XSPI_HandleTypeDef *hmspi,
...
static int stm32_mspi_read_cfg2reg(XSPI_HandleTypeDef *hmspi,
				   uint8_t nor_mode, uint8_t nor_rate, uint8_t *value)
{
	...
	s_command.Address = SPI_NOR_REG2_ADDR1;
	s_command.DummyCycles = (nor_mode == MSPI_IO_MODE_SINGLE)
				? 0U
				: ((nor_rate == MSPI_DATA_RATE_DUAL)
					? SPI_NOR_DUMMY_REG_OCTAL_DTR
					: SPI_NOR_DUMMY_REG_OCTAL);
	s_command.DataLength = (nor_rate == MSPI_DATA_RATE_DUAL) ? 2U : 1U;
...
static int stm32_mspi_config_mem(const struct device *dev)
{
	struct flash_stm32_mspi_data *dev_data = dev->data;
	uint8_t reg[2];
	...
	if ((dev_data->cfg.io_mode != MSPI_IO_MODE_OCTAL)
		&& (dev_data->cfg.data_rate == MSPI_DATA_RATE_SINGLE)) {
		LOG_INF("External flash config is SPI|DUAL|QUAD / STR");
		return 0;
	}
	...
	if (stm32_mspi_write_enable(dev,
	...
	if (stm32_mspi_write_cfg2reg_dummy(&dev_data->hmspi,
		MSPI_IO_MODE_SINGLE, MSPI_DATA_RATE_SINGLE) != 0) {
		LOG_ERR("MSPI write CFGR2 failed");
		return -EIO;
	}
	if (stm32_mspi_mem_ready(dev,
		MSPI_IO_MODE_SINGLE, MSPI_DATA_RATE_SINGLE) != 0) {
	...
	uint8_t mode_enable = ((dev_data->cfg.data_rate == MSPI_DATA_RATE_DUAL)
				? SPI_NOR_CR2_DTR_OPI_EN
				: SPI_NOR_CR2_STR_OPI_EN);
	if (stm32_mspi_write_cfg2reg_io(&dev_data->hmspi,
	...
	k_busy_wait(STM32_MSPI_WRITE_REG_MAX_TIME * USEC_PER_MSEC);
	...
	if (dev_data->cfg.data_rate == MSPI_DATA_RATE_SINGLE) {
		if (stm32_mspi_mem_ready(dev,
		...
		if (stm32_mspi_read_cfg2reg(&dev_data->hmspi,
	...
	if (dev_data->cfg.data_rate == MSPI_DATA_RATE_DUAL) {
		if (stm32_mspi_mem_ready(dev,
...
static int stm32_mspi_mem_reset(const struct device *dev)
{
	struct flash_stm32_mspi_data *dev_data = dev->data;

#if STM32_MSPI_RESET_GPIO
	const struct flash_stm32_mspi_config *dev_cfg = dev->config;
...
#endif /* STM32_MSPI_RESET_GPIO */
	/* Wait after SWreset CMD, in case SWReset occurred during erase operation */
	k_busy_wait(STM32_MSPI_RESET_MAX_TIME * USEC_PER_MSEC);
 ...
 *   set addr = 0
 */
static int flash_stm32_mspi_erase(const struct device *dev, off_t addr,
				  size_t size)
{
	const struct flash_stm32_mspi_config *dev_cfg = dev->config;
	struct flash_stm32_mspi_data *dev_data = dev->data;
	...
	if (!mspi_address_is_valid(dev, addr, size)) {
	...
	mspi_lock_thread(dev);

	if (stm32_mspi_mem_ready(dev,
		dev_data->cfg.io_mode, dev_data->cfg.data_rate) != 0) {
		mspi_unlock_thread(dev);
		LOG_ERR("Erase failed : flash busy");
		return -EBUSY;
	}

	cmd_erase.InstructionMode    = (dev_data->cfg.io_mode == MSPI_IO_MODE_OCTAL)
					? HAL_XSPI_INSTRUCTION_8_LINES
					: HAL_XSPI_INSTRUCTION_1_LINE;
	cmd_erase.InstructionDTRMode = (dev_data->cfg.data_rate == MSPI_DATA_RATE_DUAL)
					? HAL_XSPI_INSTRUCTION_DTR_ENABLE
					: HAL_XSPI_INSTRUCTION_DTR_DISABLE;
	cmd_erase.InstructionWidth    = (dev_data->cfg.io_mode == MSPI_IO_MODE_OCTAL)
					? HAL_XSPI_INSTRUCTION_16_BITS
					: HAL_XSPI_INSTRUCTION_8_BITS;

	while ((size > 0) && (ret == 0)) {

		ret = stm32_mspi_write_enable(dev,
			dev_data->cfg.io_mode, dev_data->cfg.data_rate);
		if (ret != 0) {
		...
		if (size == dev_cfg->flash_size) {
			...
			cmd_erase.AddressMode =
				(dev_data->cfg.io_mode == MSPI_IO_MODE_OCTAL)
				? HAL_XSPI_ADDRESS_8_LINES
				: HAL_XSPI_ADDRESS_1_LINE;
			cmd_erase.AddressDTRMode =
				(dev_data->cfg.data_rate == MSPI_DATA_RATE_DUAL)
				? HAL_XSPI_ADDRESS_DTR_ENABLE
				: HAL_XSPI_ADDRESS_DTR_DISABLE;
			cmd_erase.AddressWidth = stm32_mspi_hal_address_size(dev);
				...
				if ((etp->exp != 0)
					...
					if (dev_data->cfg.io_mode == MSPI_IO_MODE_OCTAL) {
						...
						cmd_erase.Instruction =
							(stm32_mspi_hal_address_size(dev) ==
			...
			ret = stm32_mspi_mem_ready(dev, dev_data->cfg.io_mode,
						dev_data->cfg.data_rate);
		}
...
static int flash_stm32_mspi_read(const struct device *dev, off_t addr,
				 void *data, size_t size)
{
	...
	XSPI_RegularCmdTypeDef cmd = mspi_prepare_cmd(dev_data->cfg.io_mode, dev_data->cfg.data_rate);
	...
	if (dev_data->cfg.data_rate == MSPI_DATA_RATE_DUAL) {
		...
		if (dev_data->cfg.io_mode == MSPI_IO_MODE_OCTAL) {
			...
			cmd.Instruction = dev_data->cfg.read_cmd;
			cmd.DummyCycles = dev_data->cfg.rx_dummy;
	...
	ret = mspi_read_access(dev, &cmd, data, size);
...
static int flash_stm32_mspi_write(const struct device *dev, off_t addr,
				  const void *data, size_t size)
{
	...
	XSPI_RegularCmdTypeDef cmd_pp = mspi_prepare_cmd(dev_data->cfg.io_mode, dev_data->cfg.data_rate);

	/* using 32bits address also in SPI/STR mode */
	cmd_pp.Instruction = dev_data->cfg.write_cmd;
	...
	cmd_pp.Address = addr;
	cmd_pp.AddressWidth = stm32_mspi_hal_address_size(dev);
	cmd_pp.DummyCycles = 0U;

	LOG_DBG("XSPI: write %zu data", size);
	mspi_lock_thread(dev);

	ret = stm32_mspi_mem_ready(dev,
				   dev_data->cfg.io_mode, dev_data->cfg.data_rate);
	if (ret != 0) {
		mspi_unlock_thread(dev);
	...
	while ((size > 0) && (ret == 0)) {
		...
		ret = mspi_write_access(dev, &cmd_pp, data, to_write);
		if (ret != 0) {
...
static const struct flash_parameters flash_stm32_mspi_parameters = {
	.write_block_size = 1,
	.erase_value = 0xff
};

static const struct flash_parameters *
flash_stm32_mspi_get_parameters(const struct device *dev)
{
	ARG_UNUSED(dev);

	return &flash_stm32_mspi_parameters;
}

static void flash_stm32_mspi_isr(const struct device *dev)
{
	struct flash_stm32_mspi_data *dev_data = dev->data;
...
{
	struct flash_stm32_mspi_data *dev_data =
		CONTAINER_OF(hmspi, struct flash_stm32_mspi_data, hmspi);
...
static void flash_stm32_mspi_pages_layout(const struct device *dev,
...
static const struct flash_driver_api flash_stm32_mspi_driver_api = {
	.read = flash_stm32_mspi_read,
	.write = flash_stm32_mspi_write,
	.erase = flash_stm32_mspi_erase,
	.get_parameters = flash_stm32_mspi_get_parameters,
#if defined(CONFIG_FLASH_PAGE_LAYOUT)
	.page_layout = flash_stm32_mspi_pages_layout,
#endif
#if defined(CONFIG_FLASH_JESD216_API)
	.sfdp_read = mspi_read_sfdp,
	.read_jedec_id = mspi_read_jedec_id,
#endif /* CONFIG_FLASH_JESD216_API */
};
...
{
	const struct flash_stm32_mspi_config *dev_cfg = dev->config;
	struct flash_stm32_mspi_data *data = dev->data;
...
static int stm32_mspi_read_status_register(const struct device *dev, uint8_t reg_num, uint8_t *reg)
{
	XSPI_RegularCmdTypeDef s_command = {
		.InstructionMode = HAL_XSPI_INSTRUCTION_1_LINE,
...
static int stm32_mspi_write_status_register(const struct device *dev, uint8_t reg_num, uint8_t reg)
{
	struct flash_stm32_mspi_data *data = dev->data;
	XSPI_RegularCmdTypeDef s_command = {
	...
	if (reg_num == 1U) {
		...
		if (data->qer_type == JESD216_DW15_QER_S2B1v1) {
			ret = stm32_mspi_read_status_register(dev, 2, &regs[1]);
			if (ret < 0) {
				return ret;
			}
			size = 2U;
		...
		if ((data->qer_type == JESD216_DW15_QER_VAL_S2B1v1) ||
			...
			ret = stm32_mspi_read_status_register(dev, 1, &regs[0]);
			if (ret < 0) {
				return ret;
			}
...
static int stm32_mspi_enable_qe(const struct device *dev)
{
	struct flash_stm32_mspi_data *data = dev->data;
	uint8_t qe_reg_num;
	uint8_t qe_bit;
	...
	ret = stm32_mspi_read_status_register(dev, qe_reg_num, &reg);
	if (ret < 0) {
		return ret;
	}
	...
	ret = stm32_mspi_write_enable(dev, MSPI_IO_MODE_SINGLE, MSPI_DATA_RATE_SINGLE);
	if (ret < 0) {
		return ret;
	}
	...
	ret = stm32_mspi_write_status_register(dev, qe_reg_num, reg);
	if (ret < 0) {
		return ret;
	}

	ret = stm32_mspi_mem_ready(dev, MSPI_IO_MODE_SINGLE, MSPI_DATA_RATE_SINGLE);
	if (ret < 0) {
		return ret;
	}
			       ...
			       const struct jesd216_bfp *bfp)
{
	...
	if (data->cfg.io_mode != MSPI_IO_MODE_OCTAL) {
		...
		if (data->cfg.io_mode != MSPI_IO_MODE_SINGLE) {
			if (data->cfg.io_mode == MSPI_IO_MODE_DUAL) {
		...
		if (data->cfg.io_mode == MSPI_IO_MODE_QUAD) {
...
#if STM32_MSPI_USE_DMA
static int flash_stm32_mspi_dma_init(DMA_HandleTypeDef *hdma, struct stream *dma_stream)
{
	int ret;
	/*
	 * DMA configuration
...
static int flash_stm32_mspi_init(const struct device *dev)
{
	const struct flash_stm32_mspi_config *dev_cfg = dev->config;
	struct flash_stm32_mspi_data *dev_data = dev->data;
	uint32_t ahb_clock_freq;
	uint32_t prescaler = STM32_MSPI_CLOCK_PRESCALER_MIN;
	int ret;
	...
	if (ret < 0) {
		LOG_ERR("MSPI pinctrl setup failed (%d)", ret);
		return ret;
	}

	if (dev_data->cfg.dqs_enable && !dev_cfg->mspicfg.dqs_support) {
	...
	if (IS_ENABLED(STM32_MSPI_DOMAIN_CLOCK_SUPPORT) && (dev_cfg->pclk_len > 1)) {
	...
	if (IS_ENABLED(STM32_MSPI_DOMAIN_CLOCK_SUPPORT) && (dev_cfg->pclk_len > 2)) {
		if (clock_control_on(DEVICE_DT_GET(STM32_CLOCK_CONTROL_NODE),
	...
	for (; prescaler <= STM32_MSPI_CLOCK_PRESCALER_MAX; prescaler++) {
		 dev_data->cfg.freq = STM32_MSPI_CLOCK_COMPUTE(ahb_clock_freq, prescaler);

		if (dev_data->cfg.freq <= dev_cfg->mspicfg.max_freq) {
			break;
		}
	}
	__ASSERT_NO_MSG(prescaler >= STM32_MSPI_CLOCK_PRESCALER_MIN &&
			prescaler <= STM32_MSPI_CLOCK_PRESCALER_MAX);
	...
	dev_data->hmspi.Init.MemorySize = find_lsb_set(dev_cfg->flash_size) - 2;
...
#endif /* STM32_MSPI_DLYB_BYPASSED */
	...
	if (flash_stm32_mspi_dma_init(&hdma_tx, &dev_data->dma_tx) != 0) {
	...
	if (flash_stm32_mspi_dma_init(&hdma_rx, &dev_data->dma_rx) != 0) {
	...
	ret = stm32_mspi_read_jedec_id(dev);
	if (ret != 0) {
	...
	if (stm32_mspi_config_mem(dev) != 0) {
	...
	ret = mspi_read_sfdp(dev, 0, u.raw, sizeof(u.raw));
	if (ret != 0) {
	...
	while (php != phpe) {
		...
		if (id == JESD216_SFDP_PARAM_ID_BFP) {
			...
			ret = mspi_read_sfdp(dev, jesd216_param_addr(php),
					     (uint8_t *)u2.dw,
...
#define MSPI_CONFIG(n)										\
	{											\
		...
		.max_freq              = DT_INST_PROP(n, mspi_max_frequency),			\
		.dqs_support           = true,							\
		.num_periph            = 0,							\
...
static void flash_stm32_mspi_irq_config_func(const struct device *dev);

static const struct stm32_pclken pclken[] = STM32_DT_CLOCKS(STM32_MSPI_NODE);

PINCTRL_DT_DEFINE(STM32_MSPI_NODE);

static const struct flash_stm32_mspi_config flash_stm32_mspi_cfg = {
	.pclken = pclken,
	.pclk_len = DT_NUM_CLOCKS(STM32_MSPI_NODE),
	.irq_config = flash_stm32_mspi_irq_config_func,
	.flash_size = DT_INST_PROP(0, size),
	.mspicfg = MSPI_CONFIG(0),
	.pcfg = PINCTRL_DT_DEV_CONFIG_GET(STM32_MSPI_NODE),
#if STM32_MSPI_RESET_GPIO
	.reset = GPIO_DT_SPEC_INST_GET(0, reset_gpios),
#endif /* STM32_MSPI_RESET_GPIO */
};

static struct flash_stm32_mspi_data flash_stm32_mspi_dev_data = {
	.hmspi = {
		.Instance = (XSPI_TypeDef *)DT_REG_ADDR(STM32_MSPI_NODE),
		.Init = {
			.FifoThresholdByte = STM32_MSPI_FIFO_THRESHOLD,
			.SampleShifting = (DT_PROP(STM32_MSPI_NODE, ssht_enable)
	...
	.cfg = MSPI_DEVICE_CONFIG_DT_INST(0),
	MSPI_DMA_CHANNEL(STM32_MSPI_NODE, tx, TX, MEMORY, PERIPHERAL)
	MSPI_DMA_CHANNEL(STM32_MSPI_NODE, rx, RX, PERIPHERAL, MEMORY)
};

static void flash_stm32_mspi_irq_config_func(const struct device *dev)
{
	IRQ_CONNECT(DT_IRQN(STM32_MSPI_NODE), DT_IRQ(STM32_MSPI_NODE, priority),
		    flash_stm32_mspi_isr, DEVICE_DT_INST_GET(0), 0);
	irq_enable(DT_IRQN(STM32_MSPI_NODE));
}

DEVICE_DT_INST_DEFINE(0, &flash_stm32_mspi_init, NULL,
		      &flash_stm32_mspi_dev_data, &flash_stm32_mspi_cfg,
		      POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE,
		      &flash_stm32_mspi_driver_api);
