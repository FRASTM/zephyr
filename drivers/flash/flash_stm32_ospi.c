...
/* Get the EXT FLASH Base Address from the Kconfig  */
BUILD_ASSERT(CONFIG_STM32_MEMMAP_EXT_FLASH_BASE_ADDRESS > 0,
	"EXT FLASH BASE ADDRESS is not correct");
#endif /* CONFIG_STM32_MEMMAP */
/* Get the EXT FLASH Size from the dts (expressed in BITS) */
BUILD_ASSERT(DT_INST_PROP(0, size) > 0,
	"EXT FLASH SIZE is not correct");
				 ...
				 void *data, size_t size)
{
	...
	if (stm32_ospi_is_memorymapped(dev)) {
		LOG_DBG("MemoryMapped Read offset: 0x%lx, len: %zu",
			(long)(CONFIG_STM32_MEMMAP_EXT_FLASH_BASE_ADDRESS + addr),
			size);

		memcpy(data,
		       (uint8_t *)CONFIG_STM32_MEMMAP_EXT_FLASH_BASE_ADDRESS + addr,
		       size);
				  ...
				  const void *data, size_t size)
{
	...
	if (stm32_ospi_is_memorymapped(dev)) {
		...
		memcpy((uint8_t *)CONFIG_STM32_MEMMAP_EXT_FLASH_BASE_ADDRESS,
		       data,
		       size);
