...
#if DT_HAS_COMPAT_STATUS_OKAY(st_stm32_dmamux)
...
#endif /* st_stm32_dmamux */

#if DT_HAS_COMPAT_STATUS_OKAY(st_stm32_dma_v3)
uint32_t table_src_size[] = {
	LL_DMA_SRC_DATAWIDTH_BYTE,
	LL_DMA_SRC_DATAWIDTH_HALFWORD,
	LL_DMA_SRC_DATAWIDTH_WORD,
};

uint32_t table_dest_size[] = {
...
#if DT_HAS_COMPAT_STATUS_OKAY(st_stm32_dma_v3) || DT_HAS_COMPAT_STATUS_OKAY(st_stm32_dmamux)
struct stream {
	DMA_TypeDef *reg;
...
#else
	hal_ret = HAL_OSPI_Receive_IT(&dev_data->hospi, data, HAL_OSPI_TIMEOUT_DEFAULT_VALUE);
#endif
	if (hal_ret != HAL_OK) {
...
static int stm32_ospi_config_mem(const struct device *dev)
{
	...
	/* going to set the OSPI mode and STR or DTR transfer rate */
	if ((dev_cfg->data_mode == STM32_OSPI_SPI_MODE)
		&& (dev_cfg->data_rate == STM32_OSPI_DTR_TRANSFER)) {
		LOG_ERR("OSPI flash wrong config SPI/DTR mode");
		return -ENOTSUP;
	}
	...
	if ((dev_cfg->data_mode == STM32_OSPI_SPI_MODE)
		...
		LOG_INF("OSPI flash config SPI/STR mode");
		return 0;
	}

	/* going to set the SPI mode (STR or DTR transfer rate) */
...
static int flash_stm32_ospi_init(const struct device *dev)
{
	...
	ret = dma_config(dev_data->dma.dev, (dev_data->dma.channel + STM32_DMA_STREAM_OFFSET), &dma_cfg);
	if (ret != 0) {
		return ret;
	}
	...
	/* configure the NOR from SPI/STR to ddata_mode/data_rate according to the DTS */
	if ((dev_cfg->data_mode == STM32_OSPI_SPI_MODE)
		&& (dev_cfg->data_rate == STM32_OSPI_STR_TRANSFER)) {
